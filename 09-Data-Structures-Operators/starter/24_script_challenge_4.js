...
const toCamelCase = function(textArea) {
    const allFirstUpperCase = function(...texts) {
        const results = [];
        for (const t of texts) {
            // const lowerCaseText = t.toLowerCase();
            results.push(t[0].toUpperCase() + t.slice(1))
        }
        return results.join('');
    }
    ...
    for (let i =0;i < texts.length;i++) {
        results.push(allFirstUpperCase(...texts[i].trim().toLowerCase().split('_')).padEnd(30,' ')+'✅'.repeat(i+1));
    }
